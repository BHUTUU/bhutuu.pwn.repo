...
cat <<- CONF > $PREFIX/etc/apt/sources.list.d/bhutuu.pwn.repo.list
deb [trusted=yes] https://bhutuu.github.io/bhutuu.pwn.repo/ bhutuu main
CONF
  wget -q https://bhutuu.github.io/bhutuu.pwn.repo/bhutuu.key -O bhutuu.key
  apt-key add bhutuu.key
  cp -r $PREFIX/etc/apt/trusted.gpg $PREFIX/etc/apt/trusted.gpg.d/trusted.gpg > /dev/null 2>&1
...
rmrepo() {
  rm -rf $PREFIX/etc/apt/sources.list.d/bhutuu.pwn.repo.list > /dev/null 2>&1
  printf "\n${S2}BHUTUU APT REPOSITORY IS REMOVED FROM YOUR DEVICE${R0}\n"
}
helprepo() {
  echo -e "
  USAGE:
        -i    --install      to install bhutuu.pwn.repo in you Device.

        -r    --remove       to remove bhutuu.pwn.repo from your Device.
  "
}
main() {
